...
// Scheduler/progress previews must not materialize history or identities.
function readEntry(stats, id) {
  var key = safeKey(id)
  var bindings = stats && stats.bindings
  return key && bindings && DeckState.own(bindings, key) ? bindings[key] : freshEntry()
}
...
function counts(stats, bindings, now, runId) {
  ...
  for (var index = 0; index < bindings.length; index++) {
    var item = readEntry(stats, bindings[index].id)
