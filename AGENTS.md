...
| **R3** | **Descriptor-Relative State I/O** | Read/write persistent **state** only via verified directory file descriptors with `O_NOFOLLOW`. Writes must use `0600` exclusive temporary files + `fsync` + atomic rename. Never traverse raw pathnames. Read-only **configuration** reads (`bin/app-config-json`) are exempt from `O_NOFOLLOW` only: linking a dotfiles repository into `~/.config` is ordinary, and refusing it read a whole configuration as absent. Such a read must `fstat` every component on the descriptor it already holds and refuse any not owned by the caller or root, or group/world-writable unless a sticky directory. Nothing on this path ever writes. |
