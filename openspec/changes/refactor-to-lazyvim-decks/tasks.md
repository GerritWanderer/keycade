...
- [ ] 1.3 Remove QML sources `lib/sources/HyprlandSource.qml`, `lib/sources/HerdrSource.qml`, `lib/sources/TmuxLiveSource.qml`, and `lib/sources/hyprland/`, verifying no dangling import references remain
- [ ] 1.4 Remove `neovim.json`, `tmux.json`, and `vim.json` from `assets/packs/` and remove the vim, neovim and tmux collectors from `tools/build_packs.py`, verifying `python3 tools/build_packs.py` succeeds with only the LazyVim path
